 ...
 * @author liming
 */
class LogDataExportToPdf extends LogDataExporter
{
	/**
	 * 导出状态
	 */
	 ...
	 * @param titleColunms 
	 */
	public LogDataExportToPdf(List<String> columns)
	{
		setColumns(columns);
	}
	...
	private void createPdfFile(File output)
	{
		...
		{
			//创建Document文档对象
			doc = new Document(PageSize.B4, 10, 10, 10, 10);
			//设置文档样式
		...
		catch(Exception ex)
		{
			...
			Logger.getLogger(LogDataExportToPdf.class.getName()).log(Level.SEVERE, null, ex);
		}
	}

	/**
	 * 创建一个包含表格标题的PDF文件表格对象
	 * @return 表格对象
	 * @throws Exception 
	 */
